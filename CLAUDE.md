...
The orchestrator (`00_orchestrator.ipynb`) loops over locations defined in `locations.json` and runs numbered feature-extraction notebooks (01–06) via papermill. Each notebook receives parameters (location name, lat/lon, walk radius) and writes a per-location CSV. The orchestrator merges all per-location CSVs into `csv/combined_<RUN_ID>.csv`.
...
- **06** — Census median income (optional, slow, controlled by `INCLUDE_JOKER` flag)
...
**Note:** The Flask UI currently only registers notebooks 01–06 in its `NOTEBOOKS` list. Notebooks 07–13 run via the orchestrator but are not individually controllable from the web interface.

### ML Visualization (`ML_Plot/`)

`NYC_classification.ipynb` runs logistic regression and XGBoost on the combined CSV, generating plots to `outputs/latest/`.
...
locations.json → 00_orchestrator → papermill(01..13) → per-site CSVs → combined CSV → ML_Plot
                                                                              ↑
                                                    Site_Scraper_App (web UI triggers 01–06 only)
```
